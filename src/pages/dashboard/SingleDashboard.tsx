          ...
          <Chart title="Top 10 Estados (Maior Taxa)">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={top10} layout="vertical" margin={{ top: 5, right: 35, bottom: 0, left: -10 }}>
                <XAxis type="number" hide domain={[90, (dataMax: number) => dataMax]} />
                <YAxis type="category" dataKey="state" stroke={C.axis} fontSize={10} tickLine={false} axisLine={false} width={35} />
                <Tooltip content={<CustomTooltip unit="%" />} />
                <Bar dataKey="value" name="Taxa" radius={[0, 2, 2, 0]}>
                  {top10.map((_, index) => (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={top20Piores} margin={{ top: 20, right: 10, left: -20, bottom: 60 }}>
                ...
                <XAxis 
                  ...
                  height={60}
                />
                <YAxis fontSize={9} stroke={C.axis} domain={[80, (dataMax: number) => dataMax]} allowDataOverflow />
                <Tooltip content={<CustomTooltip unit="%" />} />
                <Bar dataKey="value" name="Taxa %" radius={[2, 2, 0, 0]}>
                  {top20Piores.map((_, index) => (
                    <Cell key={`cell-${index}`} fill={pioresColors[index]} />
         ...
         <Chart title="Top 10 Municípios (Maior Cobertura — Melhores)">
           <ResponsiveContainer width="100%" height="100%">
             <BarChart data={top10Melhores} layout="vertical" margin={{ top: 5, right: 40, bottom: 0, left: -5 }}>
              <XAxis type="number" hide domain={[3, (dataMax: number) => dataMax]} />
               <YAxis type="category" dataKey="city" stroke={C.axis} fontSize={9} tickLine={false} axisLine={false} width={85} tick={WrappedYAxisTick} />
               <Tooltip content={<CustomTooltip />} />
               <Bar dataKey="value" name="Leitos / 1.000 hab" radius={[0, 2, 2, 0]}>
                 {top10Melhores.map((_, index) => (
  ...
  return (
    ...
    >
      ...
      {sequence && (
        <>
          <div
            className="absolute left-0 top-0 bottom-0 w-64 z-50 cursor-pointer flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300"
          ...
          <div
            className="absolute right-0 top-0 bottom-0 w-64 z-50 cursor-pointer flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300"
